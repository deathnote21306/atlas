...
def main() -> None:
    ...
    with SessionLocal() as s:
        for iso3, data in SYNOPSES.items():
            existing = s.execute(
                select(Synopsis).where(Synopsis.iso3 == iso3, Synopsis.approval_state.in_(
                    ["human_approved", "auto_approved_similarity", "auto_approved_stable_country"]
                ))
            ).scalars().first()
            if existing:
