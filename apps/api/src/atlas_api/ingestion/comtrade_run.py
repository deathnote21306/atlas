...
def main() -> None:
    ...
    parser.add_argument("--countries", type=str, default="all", help="Comma-separated ISO3 codes or 'all'")
    parser.add_argument("--years", type=str, default="2024", help="Comma-separated years")
    args = parser.parse_args()

    countries = list(COMTRADE_REPORTER_CODES.keys()) if args.countries == "all" else args.countries.upper().split(",")
    years = [int(y) for y in args.years.split(",")]

    print(f"Comtrade backfill: {len(countries)} countries × {len(years)} years = ~{len(countries) * len(years) * 4} requests")
